%--------------------------------------------------------------------------
% FlexSIM reconstruction script of the SIM Actin data provided at
...
%% General parameters
% -- Path and files
params.DataPath = fullfile(pwd,'6_SLM_Actin_3.tif');    % Path to the SIM stack
...
try
    if ~exist(params.DataPath, 'file')
        ...
        y = double(loadtiff([pwd,'/Supplementary code and data/Test/RawImage/Ours/561_3.tif'])); 
...
params.res = 108.3;    % Pixel size (nm)
...
% -- OTF Attenuation
params.OTFAttStr=0.97;            % Strenght of the OTF attenuation (in [0,1]). If 0 no OTF attenuation.
...
params.mu =  2e-5;                % Regularization parameter

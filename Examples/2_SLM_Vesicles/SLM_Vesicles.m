%--------------------------------------------------------------------------
% FlexSIM reconstruction script of the SIM Vesicle data provided at
...
%% General parameters
% -- Path and files
params.DataPath = fullfile(pwd,'2_SLM_Vesicles.tif');    % Path to the SIM stack
...
try
    if ~exist(params.DataPath, 'file')
        ...
        y = double(loadtiff([pwd,'/HiFi-SIM_v1.01/TestData/2D-SIM(3 angles_3 phases)_9 frames_Group2.tif']));
...
params.ringRegionSearch = [0.4 1];% Lower and upper limits of Fourier ring region to search peaks (given as factor of the cutoff freq)
...
% -- Dark-Sectioning (uncomment below and comment OTF Att to switch)
% params.DarkSec = 1;             % If >0 activate the Dark-sectioning (1 for middle and 2 for strong out-of-focus) 
% params.DarkSecThres = 5;        % Threshold used to define a mask on the image that contains the background
...
params.regType=3;                 % Choice regul: 1 for Tikhonov (i.e., Wiener), 2 for Total Variation, 3 for Good roughness

...
%% General parameters
% -- Path and files
params.DataPath = fullfile(pwd,'6_SLM_Nucleus_1.tif');  % Path to the SIM stack
...
try
    if ~exist(params.DataPath, 'file')
        ...
        y = double(loadtiff([pwd,'/Supplementary code and data/Test/RawImage/Ours/405_1.tif'])); 
...
% -- Background noise
params.SzRoiBack=257;       % Size (odd number or empty) of the ROI for background estimation (position automatically detected so as to minimize the intensity within the ROI)
...
params.ringMaskLim = [0,1];       % Lower and upper limits of Fourier ring mask (given as factor of the cutoff freq)
...
params.mu =  5e-3;                % Regularization parameter

%--------------------------------------------------------------------------
% FlexSIM reconstruction script of the mitochondria cristae sample provided
...
%% General parameters
% -- Path and files
params.DataPath = fullfile(pwd,'8_Stand_MitoCristae.tif');   % Path to the SIM stack 
params.pathToFlexSIM = '../../';                             % Path to the root of GitHub FlexSIM repo
try
    if ~exist(params.DataPath, 'file')
        websave(params.DataPath, 'https://github.com/LabPresse/B-SIM/raw/refs/heads/main/example_3_mitochondria_cristae/raw_images.tif');
    end
catch
...
% -- OTF Approximation
params.lamb = 569;        % Emission wavelength (nm)
...
% -- OTF Attenuation
params.OTFAttStr=0.7;           % Strength of the OTF attenuation (in [0,1]). If 0 no OTF attenuation.
...
params.mu = 7e-5;                % Regularization parameter

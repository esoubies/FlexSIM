%--------------------------------------------------------------------------
% FlexSIM reconstruction script of the hela cells mitochondria sample provided
...
%% General parameters
% -- Path and files
params.DataPath = fullfile(pwd,'8_Stand_HelaMito.tif');   % Path to the SIM stack 
params.pathToFlexSIM = '../../';                             % Path to the root of GitHub FlexSIM repo
try
    if ~exist(params.DataPath, 'file')
        websave(params.DataPath, 'https://github.com/LabPresse/B-SIM/raw/refs/heads/main/example_2_hela_cells_mitochondria/raw_images.tif');
    end
catch
...
params.SzRoiBack=41;        % Size (odd number or empty) of the ROI for background estimation (position automatically detected so as to minimize the intensity within the ROI)
...
% -- OTF Approximation
params.lamb = 660;        % Emission wavelength (nm)
params.res = 65;          % Pixel size (nm)
params.Na = 1.3;         % Objective numerical aperture

%--------------------------------------------------------------------------
% FlexSIM reconstruction script of the mitochondria sample provided
...
%% General parameters
% -- Path and files
params.DataPath = fullfile(pwd,'9_Stand_Mito.tif');   % Path to the SIM stack 
params.pathToFlexSIM = '../../';                             % Path to the root of GitHub FlexSIM repo
try
    if ~exist(params.DataPath, 'file')
        websave([pwd,'/tmp.rar'], 'https://figshare.com/ndownloader/files/44375816');
        ...
        y = double(loadtiff([pwd,'/Mito/Mito_525nm_65nm_1.49NA.tif']));

%--------------------------------------------------------------------------
% FlexSIM reconstruction script of the SIM Argolight data provided at
...
%% General parameters
% -- Path and files
params.DataPath = fullfile(pwd,'7_Stand_Argolight.tif');    % Path to the SIM stack
...
try
    if ~exist(params.DataPath, 'file')
        ...
        y = double(loadtiff([pwd,'/directSIM_V1.0/TestData/Group2/Rawdata/1.tif'])); 
        for ii=2:9
            y(:,:,ii) = double(loadtiff([pwd,'/directSIM_V1.0/TestData/Group2/Rawdata/',num2str(ii),'.tif'])); 
        end
...
% -- OTF Attenuation
params.OTFAttStr=0.9;             % Strenght of the OTF attenuation (in [0,1]). If 0 no OTF attenuation.

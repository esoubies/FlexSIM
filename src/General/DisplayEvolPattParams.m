...
for ii=1:params.nbOr
    ...
    if params.cstTimePatt || params.rollAvg>0, plot(squeeze(sqrt((k(ii,1,:)/(2*params.Na)*params.lamb/pi).^2 + (k(ii,2,:)/(2*params.Na)*params.lamb/pi).^2)),'rx','markersize',8,'linewidth',2);end
    title(['Orientation #',num2str(ii)]);grid; set(gca,'fontsize',14);
    xticks(1:params.nframes);%axis([1 params.nframes params.ringRegionSearch(1) params.ringRegionSearch(2)]);
...
for ii=1:params.nbOr
    ...
    if params.cstTimePatt || params.rollAvg>0, plot(squeeze(atan(k(ii,2,:)./k(ii,1,:))),'rx','markersize',8,'linewidth',2); end
    grid; set(gca,'fontsize',14); %axis([1 params.nframes -pi/2 pi/2]);
    xticks(1:params.nframes);
...
for ii=1:params.nbOr
    ...
    if params.cstTimePatt || params.rollAvg>0, plot(squeeze(phase(ii,1,:)),'rx','markersize',8,'linewidth',2); end
    xlabel('Frames');grid; set(gca,'fontsize',14);
    xticks(1:params.nframes); % axis([1 params.nframes 0 pi]);
    if ii==1, ylabel('Phase offset'); end
end
if params.cstTimePatt, Lgnd = legend('Estimated','Average (all frames)');
elseif params.rollAvg>0, Lgnd = legend('Estimated',['Rolling Average (size ',num2str(params.rollAvg),')']);
else  Lgnd = legend('Estimated','Average (all frames)'); end
legend('NumColumns',2);

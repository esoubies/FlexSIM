function [c, g] = evalJ(ktest, wf, G, params, filt, att_filt, grad)
...
%     if ~params.UseAllImages | (params.UseAllImages & params.eq_ph)
    if ismember(params.method, [0, 2])
        ...
        if grad           
            g(1) = - (A*[s(2);-s(1)].*2.*repmat(params.X(:), nb_imgs, 1))'*(A*s-G(:))/numel(G);
            g(2) = - (A*[s(2);-s(1)].*2.*repmat(params.Y(:), nb_imgs, 1))'*(A*s-G(:))/numel(G);
        end 
    else
    ...
    for ith_img = 1:nb_imgs
        ...
        s = AA\A'*G_i(:);
        if grad                                    
            g(1) = g(1) - (A*[s(2);-s(1)].*2.*params.X(:))'*(A*s-G_i(:))/numel(G_i);
            g(2) = g(2) - (A*[s(2);-s(1)].*2.*params.Y(:))'*(A*s-G_i(:))/numel(G_i);
            g = g';
        end 
